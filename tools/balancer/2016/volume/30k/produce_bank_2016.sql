-- Для bank коллекции
DROP TABLE IF EXISTS bank_train_balanced_2016_30k;
CREATE TABLE bank_train_balanced_2016_30k AS
...
INSERT INTO bank_train_balanced_2016_30k(twitid, text, sberbank)
    SELECT twitid, text, rank FROM mtsd_positive limit(25600);
INSERT INTO bank_train_balanced_2016_30k(twitid, text, sberbank)
    SELECT twitid, text, rank FROM mtsd_negative limit(24600);
INSERT INTO bank_train_balanced_2016_30k(twitid, text, sberbank)
    SELECT twitid, text, rank FROM mtsd_neutral limit(19600);

-- Проверяем баллансировку
SELECT COUNT(*) FROM bank_train_balanced_2016_30k WHERE
